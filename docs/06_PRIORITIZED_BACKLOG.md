...
- **Fixture-first operator preview checkpoint:** feature source `dbfbef3` plus the release-safe DWM link at `4f251b7` turns the shipped Win32 adapter into a visibly distinct Default 2.0 bridge with explicit Live / Studio / Setup workspaces, an Ember-dark semantic control pass, persistent workspace navigation, larger operator actions, color-coded Static Look swatches, and Live double-click launching. Studio Fixture Profiles now has source/footprint badges, readable mapping validation, a structured channel add/replace assistant, immutable manual-backed IR-4 6CH/10CH restoration, and a guarded Local-profile White/Amber correction that preserves all offsets/ranges/defaults. The Patch picker exposes footprint and W/A channel positions. Autoloops adds a saved-Look quick-step builder instead of requiring raw IDs. The app now exposes the existing conservative SoundSwitch 2.10.x V1 converter as a separate output-disabled candidate flow plus a seven-area source/project/safety review. Unsigned installer `0.1.0-preview.86.0` is contract-tested from exact source `4f251b717ccc3d248814c15f11dfdabf71fc786a` at SHA-256 `744de642990fba3ecb22e9f6bebc0ac4c8bbd73df5d1ad98e666f4bfb45fd087`. This is a high-impact legacy-bridge preview, **not** the `.emberskin` runtime, Designer, general fixture catalog, broad binary decoder, installed-Windows proof, or physical fixture qualification. See `handoffs/BACKYARD_PARTY_OPERATOR_PREVIEW_HANDOFF_2026-08-12.md`.
...
- **FX-3 — Implemented:** Validate Look count/name/fade/assignment capacity, duplicate names, all-Release no-ops, closed masters, and fixture-property support; include MIDI in Look deletion dependency reports. Profile authoring now uses a complete stable semantic catalog, reports mapped/open/chart-defined/safety/custom rows, and can atomically rebind every fixture from an immutable source to a compiled Local duplicate.
...
- **FX-5 — Implemented foundation / P1 surface continuation:** The shared parameter catalog classifies every current semantic by category, control kind, safety, fine-channel support, and surface availability. Projects now persist multiple named capability ranges per physical 8-bit channel plus preferred, blackout, highlight, direction, role, access, and head/cell owner metadata; compiler/Runner use bounded immutable mappings. The Studio editor and QLC+/OFL bridge preserve those rows without typed internal IDs. Add named slot pickers to every Static Look/Autoloop/Live/controller surface, richer neutral/home semantics, and a representative conformance corpus. Unknown/protected functions remain fail-closed.
