...
- **Capability-aware fixture-channel candidate:** the Preview 91 lane persists multiple named, non-overlapping semantic ranges on one physical 8-bit DMX channel, compiles them into bounded Runner mappings, preserves old profiles, imports QLC+/OFL capability rows as unreviewed evidence, and exposes a dedicated Studio range editor with generated controller binding paths, safety/access roles, owner/head/cell labels, and blackout/highlight values. Protected reset/service/custom ranges cannot render or bind; two same-layer semantic owners of one byte fail closed. Windows source compilation and portable regression tests pass; installed-Windows and physical-fixture observations remain release evidence gates. See `39_MULTI_CAPABILITY_FIXTURE_CHANNEL_CHECKPOINT.md`.
