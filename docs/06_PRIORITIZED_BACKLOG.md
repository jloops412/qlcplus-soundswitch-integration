...
- **FX-5 — P1 foundation / richer persistence pending:** The shared parameter catalog classifies every current semantic by category, control kind, safety, fine-channel support, and surface availability; generic direct defaults fail closed for chart-defined functions. Persist multiple named capabilities/ranges per physical channel plus explicit neutral/open/home/blackout/highlight values. Do not expose unknown wheel, macro, program, switching, or service functions as generic linear sliders.
