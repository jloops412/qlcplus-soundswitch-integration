...
| ID | SoundSwitch capability to match or exceed | Status | Acceptance evidence |
| --- | --- | --- | --- |
| MID-01 | MIDI Learn and mapping for any available controller | Partial | Device-agnostic mapping engine, WinMM port selection, native MIDI Learn, persistence, and compilation exist; named Track Scripts can be started or cleared, a named group can drive any normalized property, an Autoloop bank can be selected, and all transient fixture/group manual overrides can be released from MIDI alongside Looks, Autoloops, and individual-fixture properties. Windows hardware/hot-plug qualification remains. |
