...
| FIX-03 | Create and edit local fixture profiles | Partial | Native Windows profile editor persists semantic channel maps and compiles them; QXF/OFL modes import as inspectable read-only profiles that can be duplicated locally. The mapping assistant uses grouped semantic parameters, safe direct defaults, ordered rows, structure/capability audit, and exact Patch usage. Saving a duplicate can atomically rebind every affected fixture after project validation and compilation. White/Amber uses the same generic Color mapping flow as every other emitter. Rich named multi-capability ranges, immutable revisions, qualification invalidation, and physical evidence remain. |
...
| FIX-05 | RGB, RGBW, RGBA, RGBWAUV, CMY, color-wheel, dimmer, shutter, and strobe functions | Partial | A stable renderer-neutral catalog exposes all current color/intensity/position/beam/image/effect/atmosphere/custom lanes to Profiles, Looks, Autoloops, Live overrides, and controllers with explicit safety and DMX-chart policies. Defaults, constant/discrete/active-range channels, inversion, QXF emitter/preset conversion, and validation pass; persisted named capabilities/ranges and a representative conformance corpus remain required. |
...
| ID | SoundSwitch capability to match or exceed | Status | Acceptance evidence |
| --- | --- | --- | --- |
| MID-01 | MIDI Learn and mapping for any available controller | Partial | Device-agnostic mapping engine, WinMM port selection, native MIDI Learn, persistence, and compilation exist; named Track Scripts can be started or cleared, a named group can drive any normalized property or literal-zero blackout, Autoloop banks can be selected/enabled/restored, and all transient fixture/group manual overrides can be released from MIDI alongside Looks, Autoloops, and individual-fixture properties. Windows hardware/hot-plug qualification remains. |
