...
| RUN-10 | Clear DJ, interface, clock, active-loop, bank, and fault status feedback | Partial | Native Live exposes Runner, clock, adapter, frame, p99/max jitter, and the pageable active Autoloop navigation-bank mask; Diagnostics includes the exact 64-bit bank mask. A JSON qualification tool records sustained health. Durable session logging, usability, and fault-injection acceptance remain. |
