...
| RUN-08 | Live Override FX for color, movement, intensity, strobe, white/UV, blackout, and effect actions | Partial | A dedicated Live Overrides page now applies or releases any normalized property on an active-package fixture, exposes a Runner-counted active override total, and can release the entire transient ManualOverride layer. MIDI action/property routing, raw-zero blackout, work light, explicit hazard arming, and editable safety caps remain. Effect-oriented controls, hardware qualification, and richer color/movement tooling remain. |
