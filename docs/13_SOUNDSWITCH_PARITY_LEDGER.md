...
| OUT-05 | Interface hot-plug, disconnect/reconnect, and visible status | Partial | Network, DMX USB Pro, and SoundSwitch Micro outputs retry independently. Diagnostics separates Micro open state, accepted/failed writes, last Windows error, and nonzero slot count; stale queued frames are superseded. The installed Hardware Test now guides and records Windows disconnect observation, 60-second re-detection, production-session reinitialization, compiled red output, and blackout after replug, with fail-closed gate classification. Operator evidence and broader adapter tests remain. |
