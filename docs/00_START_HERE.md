...
- **First replacement-shell implementation checkpoint (2026-08-14):** the renderer-neutral Fixtures + Static Looks shell model, native tests, and an opt-in Slint 1.17.1 product-shaped lab now exist. The lab presents profile provenance, patch targets, visual fixture controls, Static Look ownership, search, guarded drafts, generation-checked commit/Undo/Redo, optional atomic project save/history through `--project`, asynchronous simulation, and Advanced-only raw diagnostics through existing domain services. The default launch opens no output. An explicit `--project` plus `--allow-physical-preview` launch can exercise the existing bounded selected-target Runner lease with a visible 35% cap, 30-second countdown, exact state/fault feedback, coalesced draft updates, and terminal **Blackout & stop**. It is not installed and does not select Slint as the production toolkit. See `45_SLINT_FIXTURES_LOOKS_LAB_CHECKPOINT.md`, D-092, and D-094.
- **Canonical Static Look activation and preview feedback (2026-08-14):** registry `1.8.0`, generation 2, digest `e63bf41dd8548a26e2c83c38ab8c98f209096ac1c15b84f225b3831c3d000ed6` retains the complete package/activation generation, owner, behavior, status, and transition tuple for Live activation and adds two Studio-only preview commands plus eight coherent preview states. The registry now contains 31 commands and 54 states and remains compatible-additive against V1. Preview start/stop are intentionally unavailable to MIDI, keyboard, and Ember Actions; a trusted Studio host is the only physical-preview entry. Neither registry change advances the product installer line.
