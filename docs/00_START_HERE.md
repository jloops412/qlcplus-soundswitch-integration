...
- **First replacement-shell implementation checkpoint (2026-08-14):** the renderer-neutral Fixtures + Static Looks shell model, native tests, and an opt-in Slint 1.17.1 product-shaped lab now exist. The lab presents profile provenance, patch targets, visual fixture controls, Static Look ownership, search, guarded drafts, generation-checked commit/Undo/Redo, optional atomic project save/history through `--project`, preview simulation, and Advanced-only raw diagnostics through existing domain services. It is not installed, opens no output, and does not select Slint as the production toolkit. See `45_SLINT_FIXTURES_LOOKS_LAB_CHECKPOINT.md` and D-092.
- **Canonical Static Look activation feedback (2026-08-14):** registry `1.7.0`, generation 2, digest `a5d3ed5cc0f60c8caaa756c63d848dcb961d1d87c9761b3897a73ef28b010a3b` expands the shared Live state from active Look ID alone to package generation, activation generation, owner kind/key, behavior, status, and transition progress. Activate/Toggle/Hold/Clear publish the complete feedback tuple; `staticLook.hold` is now implemented through the trusted generation-safe owner path. Live projections copy this authoritative snapshot so future skins, MIDI/controllers, and remotes do not infer ownership or completion locally. The change is compatible-additive, raises the canonical state count from 39 to 46, and does not advance the product installer line.
