...
Backend descriptors separately record implementation stage, evidence stage, capabilities, advertised hardware universe count, EmberLights-supported universe count, and a Normal/ExperimentalOptIn/Unavailable configuration policy. Host-accepted writes, loopback delivery, physical DMX response, and gig qualification remain distinct evidence levels.

- SoundSwitch Micro, DMX USB Pro, Art-Net, and sACN use the implemented contract today.
- SoundSwitch Control One's standard MIDI surface remains independent from its two DMX outputs. The Windows JLC1/JLS1 transport verifies `VID_15E4/PID_0054`, configuration 1, interface 0, bulk OUT `0x01`, sends the four established startup controls, and routes complete 522-byte frames to zero-based ports 0 and 1. The production Runner maps U1/U2 to jacks 1/2, retries through the shared lifecycle, blackouts on stop/close, and publishes the shared health contract. Connections exposes it only as an explicit experimental/unqualified opt-in until the installed two-jack/blackout/replug procedure passes.
