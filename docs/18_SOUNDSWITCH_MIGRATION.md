...
In EmberLights, use **File → Inspect SoundSwitch Project** to create a JSON inventory, or **File → Create SoundSwitch Migration Bundle** to make a verified copy. The bundle contains:
...
Provide one verified bundle from the SoundSwitch version you use, ideally containing a small known venue with one fixture, one Static Look, one Autoloop, and one deliberately simple scripted track. A second export after changing exactly one item lets us infer fields by controlled comparison. Copied audio is needed only for validating track identity/relinking; keep your originals and share only material you are authorized to use.
