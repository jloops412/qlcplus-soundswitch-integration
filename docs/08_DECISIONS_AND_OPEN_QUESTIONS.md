...
| D-035 | SoundSwitch migration begins with a read-only, SHA-256 inventoried source bundle; semantic decoding remains sample-corpus gated. | Official guidance separates project data, lightshow data, and copied audio. Independently observed binary layouts are useful evidence but are insufficient for a lossless production decoder across versions. Unknown payloads must remain byte-identical and third-party unlicensed decoder code is not incorporated. |
