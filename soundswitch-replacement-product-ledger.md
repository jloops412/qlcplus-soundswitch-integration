...
- The Windows release workflow now treats the CMake install tree as authoritative, binds every payload file to the exact checked-out commit with deterministic path/size/SHA-256 evidence, verifies staged/portable/installed equivalence, retains a non-outputting installed qualification report, checks file association and uninstall cleanup, and emits format-2 release evidence. The real Windows job/artifact has not run for this unpushed head; Authenticode, cross-version upgrade/rollback, supported-machine coverage, and hardware qualification remain open.
...
- No physical/gig, installed-Windows, source-decoder fidelity, complete Action-platform, complete SoundSwitch-parity, or migration-decoder claim is made. GitHub reservation/evidence checkpoints are published, but PR #53/#82/#83 and the local integration commits remain unpublished because `gh`, required by the repository code-publish workflow, is not installed in this workspace; no downloadable artifact exists for this head.
