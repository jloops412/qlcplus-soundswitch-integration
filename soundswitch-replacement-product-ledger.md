...
- Named fixture functions are one shared semantic capability surface. Static Looks, Live, Autoloop authoring, MIDI/controllers, Ember Actions, and skins consume the same stable choices and safety/access policy; none may create a vendor-, controller-, or skin-specific fixture engine.
- Migration readiness is recorded by source and lifecycle stage. A matching artifact/hash establishes identity only, not semantic decoder fidelity. WOLFMIX remains a later evidence-gated source and cannot be called imported until an authorized versioned controlled-delta corpus supports an actual parser and deterministic regression suite.
...
## 2026-08-13 Preview 93 named-function portability checkpoint

- The stable profile-backed fixture choice catalog now drives four product surfaces without changing the engine: Static Looks, Live Overrides, committed Autoloop V2 event authoring, and persistent MIDI/controller mapping.
- Autoloop authoring uses exact half-open 960-PPQ semantic `PropertyBlock` events. A mixed-profile group expands to per-fixture targets/lanes/events so different wheel or range layouts retain their own normalized values; the complete candidate validates and commits as one generation-checked transaction only after output-disabled production preview in the transitional Windows workflow.
- The controller planner derives the existing `SetProperty` / `SetGroupProperty` action, semantic endpoint range, and stable named-choice provenance from a learned device gesture. Homogeneous groups remain one action; mixed groups expand atomically within fixed project and actions-per-message budgets. Format-1 MIDI records reuse one previously reserved field for the stable choice ID and preserve old projects with an empty value.
- Protected functions never enter the catalog. Safety-gated choices remain fail-closed without explicit authoring confirmation and are hidden from the current quick-pick adapters. The services contain no Win32, controller-vendor, raw-DMX, output-adapter, or skin runtime authority.
- A deterministic content-safe portability review now reports Probe, Inventory, Decode, Reconcile, Plan, Commit, and Upgrade separately. SoundSwitch source binding/artifact identity remains distinct from semantic decoding and imported-object fidelity. Every WOLFMIX stage remains research/evidence unavailable under `wolfmix.controlled_delta_corpus_unavailable`; no WOLFMIX source inspector, parser, importer, conversion, or runtime support is claimed.
- Warnings-as-errors Make `all` and its complete 30-executable test surface pass; a fresh CMake/CTest build also registers and passes `autoloop_fixture_controls_tests`, `fixture_controller_binding_tests`, and `migration_portability_review_tests`. Full Windows installer/package, installed application, physical fixture/controller, decoder corpus, soak, and gig evidence remain separate gates.
- Unsigned Windows x64 installer `0.1.0-preview.93.0` is contract-tested from exact source `375348e72ec37f7ed302f0cdc52d7c7dacaff865`. Seventeen package-contract tests pass; the 18-file stage plus manifest verifies; repeat NSIS output is byte-identical; and all 19 extracted payload files match. Installer SHA-256 is `cda9457c6c2b3aca8c1624d52fe19ab36088c08e070b628aefaa4efd2864a69f`; manifest SHA-256 is `00d77eef6afe39db9483192528d992bd445734916a1f63798988f00c1b15aec4`. Native install/upgrade/launch/uninstall and hardware evidence remain deferred.

## 2026-08-13 Preview 94 profile-workbench and modular-control checkpoint

- Studio Fixture Profiles now exposes a general modeless channel-map workbench backed by the shared 53-parameter catalog. A Local draft can add a safe direct parameter at the next open slot, fill unused footprint slots with explicit zero constants, inspect named DMX ranges, and review an exchange between two compatible ordinary direct-channel functions without typing persistence IDs. Built-in/imported snapshots remain read-only until duplicated, and Save retains the existing validated optional Patch rebind.
- White and Amber remain ordinary profile mappings. Focused compile/render coverage proves an IR-4-style correction produces White on CH5 and Amber on CH4 after exchanging CH4/CH5 semantics. This is software evidence only; the active fixture mode, patched snapshot, address, interface, and physical response still require operator verification. A running Live package intentionally keeps its immutable snapshot until Stop Show / Start Show after a saved profile/patch revision.
- The canonical UI registry advances compatibly to set `1.2.0`, generation `2`, digest `0a647969b836a52106395709a8d83e5b22126f3c173d52466f9d056d4bf83699`. The accepted 29 commands and 39 states are unchanged; two non-callable component contracts describe the profile editor and fixture-function browser, bringing the component count to seven without adding a Runner path.
- A toolkit-neutral fixture-function component now produces bounded searchable/category-ready stable-ID rows, exact coverage/raw-range diagnostics, accessibility text, and typed existing fixture/group override invocations after fresh re-resolution. It never dispatches or accesses devices. A deterministic Ember Action planner compiles the same exact choices to the existing registered set/release commands for one fixture or a semantically uniform complete group and fails closed for mixed, partial, stale, protected, or unconfirmed safety-gated input. Persisted Action activation, `MapValue`, skin-runtime consumption, and a visual binding editor remain open.
- The generated Action registry adapter now materializes canonical enum values referenced through `value.fixtureProperty`; the Windows build uses the portable bounded decimal parser. Warnings-as-errors Make `all`, the complete 32-executable Make test surface, a fresh focused CMake/CTest 6/6 run, and the Windows x64 warnings-as-errors application/package build pass. Clean source `5c1f1f4b98343d08c82e9201af0fde4d20d1794b` produced unsigned installer `0.1.0-preview.94.0` at SHA-256 `69eacbe90be204620b92f0ec9d35120af738f6c729898ad966ad71deb5a095ef`; all 17 package-contract regressions, repeat-NSIS identity, archive extraction, and 19-file stage comparison pass. Installed-Windows lifecycle and physical hardware evidence remain deferred.

## 2026-08-13 Preview 95 actual Runner-frame and IR-4 bench checkpoint

- The production renderer now emits fixed per-channel attribution beside each two-universe frame: runtime fixture/mapping/capability indices, semantic property, value mode, winning layer, encoding/fine role, and explicit default/constant/property/capability/conflict/safety origin. The scheduler retains fixed work, allocation-free rendering, and its existing SPSC output boundary.
- The output thread publishes a bounded immutable latest snapshot after routing. It preserves generation, sequence, monotonic render time, pre-global-blackout bytes, actual routed bytes, blackout application, and per-backend configured/attempted/accepted/error results. Snapshot readers cannot mutate Runner or create another output path.
- A toolkit-neutral inspector resolves fixed runtime evidence against the current project, produces exact full-frame/per-universe SHA-256 values and bounded nonzero rows, diagnoses stale/missing/invalid/truncated evidence, and compares raw full-universe or one-hot references with cause-labelled differences. System Diagnostics adapts this report and preserves operator scroll/selection during its timed refresh.
- The immutable IR-4 qualification project now covers Blackout/R/G/B/W/A with complete 512-channel and JLS1 packet comparisons, no CH6/UV activation, and no unrelated universe output. A separate packaged Local operator clone is output-disabled by default, editable through the general 53-parameter channel workbench, and includes the same six Static Looks. Diagnostics automatically compares an active bench look with the manual reference so CH4 White/CH5 Amber versus any saved exchange is explicit.
- Warnings-as-errors Make test passes the complete 33-executable surface; a fresh Release CMake build and CTest pass 38/38; all 18 package-contract regressions and a fresh Windows x64 Zig/Clang warnings-as-errors application/tool build pass. Clean source `68cc13be7c0b7d9735609b8ca7533be762b90094` produced unsigned installer `0.1.0-preview.95.0` at SHA-256 `a099852f1fe5642d55eb69e2cd4f4653fd94e367b73703af14b77160ad5e2dac`; the 20-file stage plus manifest verifies exactly against the 21 extracted payload files, and repeat NSIS output is byte-identical. Payload manifest SHA-256 is `2d35c338e363b535b08e8231d8e11fa685e7510f5b3017f5f51ae2a41665e82e`. Native installed-Windows lifecycle, successful raw-attempt binding, both physical IR-4 observations, blackout/spill/reconnect/endurance, and gig qualification remain open. No accepted host write or matching software frame is a physical qualification claim.
