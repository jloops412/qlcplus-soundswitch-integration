...
- Added transient Live Overrides for fixtures and named groups with one-touch MIDI control: targetless Release All and normalized named-group property mappings. Groups compile to immutable deduplicated fixture-ID arrays, both actions share bounded MIDI-to-Runner paths, the release clears only the ManualOverride layer, persisted action values remain compatible, and active override counts return to Live and Diagnostics. MIDI pads can select a bank, enable/disable it through normal mapping behavior, or restore all 64 Autoloop banks through the existing Runner status mask.
