...
[[nodiscard]] std::string default_destination_ref(std::uint32_t target_id) {
    if (is_uplight(target_id)) {
        return "ir4-" + std::to_string(target_id - 197U);
    }
...
SoundSwitch2026RedSmoothProjectResult
create_soundswitch_2026_red_smooth_project(
    const std::filesystem::path& source_root,
    const SoundSwitch2026RedSmoothOptions& options) {
    SoundSwitch2026RedSmoothProjectResult result;
    auto candidate = make_safe_color_rig_v1_template();
    candidate.id = "soundswitch-2026-red-smooth-output-disabled";
    candidate.name =
        "SoundSwitch 2026 - Red Smooth Slice - OUTPUT DISABLED";
    candidate.connections.artnet_enabled = false;
    candidate.connections.sacn_enabled = false;
    candidate.connections.dmx_usb_pro_ports = {};
    candidate.connections.soundswitch_micro_universe = 0U;
    candidate.connections.soundswitch_control_one_experimental = false;

    AutoloopSourceDocument empty_source;
    normalize_autoloop_source(empty_source);
    const auto reset_source =
        upsert_persisted_autoloop_source(candidate, empty_source);
    if (!reset_source) {
        result.message =
            "The output-disabled current-rig project could not reset unrelated starter Autoloops.";
        return result;
    }

    result.proposal = build_soundswitch_2026_red_smooth_proposal(
        source_root, candidate, options);
    if (!result.proposal) {
        result.message = result.proposal.message;
        return result;
    }

    StudioDocumentService document;
    const auto loaded = document.replace_document(
        document.generation(), candidate,
        StudioDocumentBoundary::OpenedDocument);
    if (!loaded) {
        result.message =
            "The current-rig migration candidate could not enter the Studio transaction: " +
            loaded.message;
        return result;
    }
    const auto before = document.snapshot();
    if (!before.autoloop_source || !before.autoloop_source.stamp.present) {
        result.message =
            "The Studio transaction could not verify the reset Autoloop source.";
        return result;
    }
    const auto applied = document.apply_autoloop_source(
        before.generation, before.autoloop_source.stamp,
        result.proposal.source);
    if (!applied) {
        result.message =
            "The generation-checked SoundSwitch import was rejected: " +
            applied.message;
        return result;
    }

    result.project = document.snapshot().document;
    result.project.unknown_records.push_back(
        "SOUNDSWITCH_2026_IMPORT\t" +
        std::string(kSoundSwitch2026ProjectId) + "\t" +
        std::string(kSoundSwitch2026RedSmoothName) + "\t" +
        result.proposal.source_digest +
        "\tdeterministically-translated\toutput-disabled");
    ...
    if (!validation.ok() || !compilation) {
        result.project = {};
        result.message =
            "The applied SoundSwitch 2026 project failed final validation or compilation.";
        return result;
    }
    result.success = true;
    result.message =
        "Created an output-disabled current-rig project with the exact Medium slot 1 "
        "Red - Smooth Pulse source slice. Uplight color context remains explicitly missing; "
        "no color was invented.";
    return result;
}
