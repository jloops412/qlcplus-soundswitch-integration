...
emberlights::StudioColorPaletteAsset make_palette_asset() {
    emberlights::StudioColorPaletteAsset palette;
    palette.id = "event-colors";
    palette.name = "Event Colors";
    emberlights::StudioColorSwatch gold{"gold", "Love & Light Gold", {}};
    gold.color.rgb = {0.843137264F, 0.694117665F, 0.521568656F};
    gold.color.amber = 0.2F;
    gold.color.intensity = 0.8F;
    emberlights::StudioColorSwatch blue{"blue", "Blue", {}};
    blue.color.rgb = {0.0F, 0.0F, 1.0F};
    blue.color.intensity = 0.75F;
    palette.swatches = {gold, blue};
    return palette;
}
...
void test_persisted_palette_transactions() {
    emberlights::StudioDocumentService service;
    const auto initial = service.snapshot();
    auto palette = make_palette_asset();
    auto candidate = initial.document;
    CHECK(emberlights::upsert_studio_palette_asset(candidate, palette) ==
        emberlights::StudioPaletteMutationResult::Added);
    CHECK(emberlights::validate_project(candidate).ok());

    const auto applied = service.apply_candidate(initial.generation, candidate);
    CHECK(applied.result == emberlights::StudioMutationResult::Applied);
    const auto first_commit = service.snapshot();
    CHECK(first_commit.generation == initial.generation + 1U);
    CHECK(first_commit.undo_count == 1U);
    CHECK(first_commit.document.color_palettes.size() == 1U);

    const auto serialized = emberlights::serialize_project(first_commit.document);
    CHECK(serialized.find("COLOR_PALETTE_V1\tevent-colors\tEvent Colors") !=
        std::string::npos);
    CHECK(serialized.find("COLOR_SWATCH_V1\tevent-colors\tgold") !=
        std::string::npos);
    CHECK(emberlights::serialize_project(first_commit.document) == serialized);
    emberlights::ProjectDocument reparsed;
    CHECK(emberlights::parse_project(serialized, reparsed));
    CHECK(reparsed.color_palettes == first_commit.document.color_palettes);
    CHECK(emberlights::serialize_project(reparsed) == serialized);

    auto no_change_candidate = first_commit.document;
    CHECK(emberlights::upsert_studio_palette_asset(no_change_candidate, palette) ==
        emberlights::StudioPaletteMutationResult::NoChange);
    const auto no_change = service.apply_candidate(
        first_commit.generation, std::move(no_change_candidate));
    CHECK(no_change.result == emberlights::StudioMutationResult::NoChange);
    CHECK(service.generation() == first_commit.generation);

    auto stale_candidate = first_commit.document;
    auto stale_palette = palette;
    stale_palette.name = "Stale Palette";
    CHECK(emberlights::upsert_studio_palette_asset(stale_candidate, stale_palette) ==
        emberlights::StudioPaletteMutationResult::Updated);

    auto updated_candidate = first_commit.document;
    palette.name = "Updated Event Colors";
    palette.swatches.front().color.white = 0.15F;
    CHECK(emberlights::upsert_studio_palette_asset(updated_candidate, palette) ==
        emberlights::StudioPaletteMutationResult::Updated);
    const auto updated = service.apply_candidate(
        first_commit.generation, std::move(updated_candidate));
    CHECK(updated.result == emberlights::StudioMutationResult::Applied);
    CHECK(service.apply_candidate(first_commit.generation, std::move(stale_candidate)).result ==
        emberlights::StudioMutationResult::StaleGeneration);

    auto after_update = service.snapshot();
    CHECK(after_update.document.color_palettes.front().name == "Updated Event Colors");
    CHECK(service.undo(after_update.generation).result ==
        emberlights::StudioMutationResult::Applied);
    const auto after_undo = service.snapshot();
    CHECK(after_undo.document.color_palettes.front().name == "Event Colors");
    CHECK(service.redo(after_undo.generation).result ==
        emberlights::StudioMutationResult::Applied);
    CHECK(service.snapshot().document.color_palettes.front().name == "Updated Event Colors");

    auto removal_candidate = service.snapshot().document;
    CHECK(emberlights::remove_studio_palette_asset(removal_candidate, "event-colors") ==
        emberlights::StudioPaletteMutationResult::Removed);
    const auto before_removal_generation = service.generation();
    CHECK(service.apply_candidate(
        before_removal_generation, std::move(removal_candidate)).result ==
        emberlights::StudioMutationResult::Applied);
    CHECK(service.snapshot().document.color_palettes.empty());
    CHECK(service.undo(service.generation()).result ==
        emberlights::StudioMutationResult::Applied);
    CHECK(service.snapshot().document.color_palettes.size() == 1U);

    auto invalid_palette = palette;
    invalid_palette.asset_version = 2U;
    CHECK(!emberlights::valid_studio_palette_asset(invalid_palette));
    auto invalid_candidate = service.snapshot().document;
    CHECK(emberlights::upsert_studio_palette_asset(invalid_candidate, invalid_palette) ==
        emberlights::StudioPaletteMutationResult::Invalid);

    auto future_project = emberlights::make_starter_project();
    future_project.unknown_records.push_back(
        "COLOR_PALETTE_V2\tfuture-palette\tFuture Palette");
    future_project.unknown_records.push_back(
        "COLOR_SWATCH_V2\tfuture-palette\tfuture\tFuture\topaque");
    const auto future_serialized = emberlights::serialize_project(future_project);
    emberlights::ProjectDocument future_reparsed;
    CHECK(emberlights::parse_project(future_serialized, future_reparsed));
    CHECK(future_reparsed.unknown_records == future_project.unknown_records);
    CHECK(emberlights::serialize_project(future_reparsed) == future_serialized);
}
...
void test_no_output_realtime_preview() {
    auto project = make_authoring_project();
    CHECK(emberlights::upsert_studio_palette_asset(project, make_palette_asset()) ==
        emberlights::StudioPaletteMutationResult::Added);
    ...
    const auto palette_preview = preview.preview_palette_swatch(
        7U, "wash-left", "event-colors", "blue");
    CHECK(palette_preview.result == emberlights::StudioPreviewResult::Applied);
    CHECK(preview.snapshot().draft_color_active);
    CHECK(near(preview.snapshot().fixtures.front().display_rgb.blue, 0.75F, 0.01F));
    const auto palette_frame = preview.snapshot().dmx_frames;
    CHECK(preview.clear_draft_color(7U));
    CHECK(preview.preview_palette_swatch(7U, "wash-left", "event-colors", "blue"));
    CHECK(preview.snapshot().dmx_frames.universes == palette_frame.universes);
    CHECK(preview.preview_palette_swatch(
        7U, "wash-left", "missing-palette", "blue").result ==
        emberlights::StudioPreviewResult::MissingContent);
    CHECK(preview.preview_palette_swatch(
        7U, "wash-left", "event-colors", "missing-swatch").result ==
        emberlights::StudioPreviewResult::MissingContent);
    CHECK(preview.clear_draft_color(7U));
...
int main() {
    ...
    test_persisted_palette_transactions();
